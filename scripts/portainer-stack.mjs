...
Optional:
  ...
  GHCR_REGISTRY_TOKEN
  GHCR_REGISTRY_NAME (default: ghcr-auto-einsmal-eins-expedition)
`);
}

const IMAGE_ENV_PRIORITY = ['EINSMAL_EINS_EXPEDITION_IMAGE', 'ELIBO_IMAGE'];
...
async function ensureGhcrRegistry(config, username, token) {
  const registryName = process.env.GHCR_REGISTRY_NAME || 'ghcr-auto-einsmal-eins-expedition';
...
async function runDeploy(config, imageRef, dryRun) {
  ...
  let imageEnvVar = detectedImageEnvVar || 'EINSMAL_EINS_EXPEDITION_IMAGE';
  let migratedLegacyEnv = false;

  if (detectedImageEnvVar === 'ELIBO_IMAGE') {
    nextStackFileContent = nextStackFileContent
      .replace(/\$\{ELIBO_IMAGE(?=[:}])/g, '${EINSMAL_EINS_EXPEDITION_IMAGE')
      .replace(/\$ELIBO_IMAGE\b/g, '$EINSMAL_EINS_EXPEDITION_IMAGE');

    if (!nextEnvMap.has('EINSMAL_EINS_EXPEDITION_IMAGE') && nextEnvMap.has('ELIBO_IMAGE')) {
      nextEnvMap.set('EINSMAL_EINS_EXPEDITION_IMAGE', String(nextEnvMap.get('ELIBO_IMAGE') || ''));
    }

    nextEnvMap.delete('ELIBO_IMAGE');
    imageEnvVar = 'EINSMAL_EINS_EXPEDITION_IMAGE';
    ...
    if (!replaced.replaced) {
      throw new Error(
        'Konnte in der Stack-Compose-Datei keine image:-Zeile finden. ' +
          'Nutze eine Compose-Datei mit image: ... oder ${EINSMAL_EINS_EXPEDITION_IMAGE}.'
      );
    }
