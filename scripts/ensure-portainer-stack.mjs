...
function usage() {
  console.log(`Usage:
  node scripts/ensure-portainer-stack.mjs --stack-name <name> [--external-port 8089] [--internal-port 8080]
                                       [--image-env-var EINSMAL_EINS_EXPEDITION_IMAGE] [--image-default ghcr.io/<owner>/einsmal-eins-expedition:main]
...
function parseArgs(argv) {
  const out = {
    ...
    imageDefault: 'ghcr.io/elfiboewer/einsmal-eins-expedition:main',
...
async function main() {
  ...
  const imageEnvVar = sanitizeImageEnvVar(args.imageEnvVar || 'EINSMAL_EINS_EXPEDITION_IMAGE');
